...
const initSpins = () => {
  const { settings, simulation } = Store.getState();
  ...
  let sBefore = new Array<any>(settings.latticeSize * settings.latticeSize);
  sBefore.fill(0);

  let spin = new Array<any>(settings.latticeSize * settings.latticeSize);

  for (let i = 0; i < settings.latticeSize * settings.latticeSize; i++) {
