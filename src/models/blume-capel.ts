...
export const colorBEG = (i: number, j: number, spins: Array<Array<number>>) => {
  let { context, width } = create(Store).getState().canvas;
...
const BlumeCapel = () => {
  const { magneticField, latticeSize, magnetism, stepsPerFrame } =
    Store.getState().settings;
  const { temperature, localMagnetic } = Store.getState().simulation;
      ...
      if (
        ...
        return (
          ...
          2.0 *
            CouplingConstant *
            spins[i2][j2] *
            (bottom2 + top2 + left2 + right2) +
          4.0 * CouplingConstant +
          2.0 * spins[i1][j1] * (magneticField! + localMagnetic[i1][j1]) +
          2.0 * spins[i2][j2] * (magneticField! + localMagnetic[i2][j2])
        );
      } else
        return (
          2.0 *
