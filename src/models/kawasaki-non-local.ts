...
export const deltaUforKawasakiforM = (
  ...
  const { magneticField, magnetism } = Store.getState().settings;
  const { localMagnetic } = Store.getState().simulation;
...
const deltaUforKawasaki = (
  ...
  const { latticeSize, magneticField, magnetism, boundariesConditions } =
  ...
  if (
    (i1 === i2 &&
      ((j1 === latticeSize - 1 && j2 === 0) ||
        (j1 === 0 && j2 === latticeSize - 1))) ||
    (j1 === j2 &&
      ((i1 === latticeSize - 1 && i2 === 0) ||
        (i1 === 0 && i2 === latticeSize - 1)))
  ) {
    if (
...
const KawasakiNonLocal = () => {
  const { latticeSize, stepsPerFrame } = Store.getState().settings;
  const { localMagnetic, temperature } = Store.getState().simulation;
  ...
  const model = () => {
    ...
    if (spins[i1][j1] !== spins[i2][j2]) {
      ...
      if (
        temperature == 0 &&
        (_EdiffforM < 0.0 || (_EdiffforM == 0 && Math.random() < 0.5))
      ) {
